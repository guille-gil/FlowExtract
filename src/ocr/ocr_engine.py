...
class OCREngine:
    ...
    def process_elements(
        ...
        """
        Extract text from all classified elements.
        
        Args:
            image_path: Path to original image
            classification_json_path: Path to classification results JSON
            output_dir: Directory to save OCR results
            
        Returns:
            OCR results dictionary
        """
        # Load image and classifications
        image = cv2.imread(image_path)
        classifications = load_json(classification_json_path)
        
        # Extract text from each element
        elements = classifications['elements']
